...
% Show bullet markers (Metropolis hides them by default)
\setbeamertemplate{itemize item}{\raise0.3ex\hbox{\scriptsize$\bullet$}}
\setbeamertemplate{itemize subitem}{\raise0.2ex\hbox{\scriptsize--}}
...
\author{Hyuntae Choi, Seongeun Kim, Jiheun Ha, Jin Lee, \\[4pt] Medina Mukhamedina, Jiyong Eom, Son H. Kim, Haewon McJeon\\[12pt]}
\date{19th March, 2026}
\institute{KAIST IAM Group}
...
\large
\begin{tabular}{@{} r @{\;\;} l @{}}
  \textbf{1.} & \textbf{\color{gray!70!black}Introduction} \\[1.0em]
  \textbf{2.} & \textbf{\color{green!50!black}Framework \& Data} \\[1.0em]
  \textbf{3.} & \textbf{\color{blue!60}Economy} \\[1.0em]
  \textbf{4.} & \textbf{\color{orange!70}Energy System} \\[1.0em]
  \textbf{5.} & \textbf{\color{purple!60}Technology Choice} \\[1.0em]
  \textbf{6.} & \textbf{\color{yellow!70!black}Results \& Next Steps} \\
\end{tabular}
\end{frame}
...
\begin{tabular}{@{} l l l @{}}
\toprule
& \multicolumn{1}{c}{\textbf{\normalsize Before}} & \multicolumn{1}{c}{\textbf{\normalsize GHIM}} \\
\midrule
\rowcolor{blue!6}
...
\column{0.48\textwidth}
\begin{tcolorbox}[goalbox, colback=green!8, colframe=green!50!black, title={\color{white}2\quad Coupled Energy-Economy w/ Foresight}]
...
\scriptsize Easy to adjust resolution and nesting w/ data provided.
\end{tcolorbox}
\vspace{0.15em}
...
\toprule
\textbf{Component} & \multicolumn{1}{c}{\textbf{\small Phase 1}} & \multicolumn{1}{c}{\textbf{\small Phase 2+}} \\
\midrule
...
\begin{tikzpicture}[
  ...
  % ═══ References (top-left, accumulate with overlays) ═══
  \node<1->[anchor=west, font=\scriptsize, text=gray!60] at (-5.8, 0.0)
    {Riahi et al.\ 2017};
  \node<2->[anchor=west, font=\scriptsize, text=blue!60] at (-5.8, -0.40)
    {Jorgenson \& Griliches 1967};
  \node<3->[anchor=west, font=\scriptsize, text=blue!40] at (-5.8, -0.80)
    {Clarke \& Edmonds 1993};
  \node<4->[anchor=west, font=\scriptsize, text=red!50] at (-5.8, -1.20)
    {Rogner 1997};
  \node<5->[anchor=west, font=\scriptsize, text=purple!50] at (-5.8, -1.60)
    {Calvin et al.\ 2019};
  \node<9->[anchor=west, font=\scriptsize, text=SpruceGreen!70] at (-5.8, -2.00)
    {Edmonds \& Reilly 1985};
  \node<11->[anchor=west, font=\scriptsize, text=red!40!gray] at (-5.8, -2.40)
    {Nordhaus 1993};
  \node<11->[anchor=west, font=\scriptsize, text=red!40!gray] at (-5.8, -2.80)
    {Burke et al.\ 2015};
\end{tikzpicture}
\end{frame}
...
\begin{frame}[t]{KLEM Economy (Jorgenson \& Griliches, 1967; Timmer et al., 2007)}
\begin{columns}[T]
...
\begin{tikzpicture}[
  ...
  \node[lbl, left=1pt of KL, xshift=-2pt, align=right] {$\sigma_{K,L}$\\$<\!1$};
...
\visible<2->{%
\begin{tcolorbox}[infobox, colback=blue!4, colframe=blue!50, title={Why Nested CES?}, top=2pt, bottom=2pt]
\footnotesize
\footnotesize
Each nest has its own $\sigma$ --- heterogeneous substitution.\\
Carbon tax $\to$ elec replaces fossil first ($\sigma\!>\!1$, easy),\\
\phantom{Carbon tax} $\to$ then total energy falls ($\sigma\!<\!1$, hard).
\end{tcolorbox}}

\vspace{0.1em}
\visible<3->{%
\begin{tcolorbox}[infobox, colback=green!6, colframe=green!50!black, title=Key Equations, top=1pt, bottom=1pt]
\footnotesize
\textbf{Gross Output:}\quad $Q = \mathrm{CES}(KLE,\; M)$

\vspace{1pt}
\textbf{GDP:}\quad $Y = Q - p_E \!\cdot\! E - p_M \!\cdot\! M$

\vspace{1pt}
\textbf{Energy:}\quad $E$ scales with output, falls when $p_E$ rises

\vspace{1pt}
\textbf{Budget:}\quad savings $=$ capital $+$ energy $+$ R\&D investment

\vspace{2pt}
\textbf{Feedback:}\; $p_E \!\uparrow\;\Rightarrow\; E\!\downarrow\;\Rightarrow\; Q\!\downarrow\;\Rightarrow\; Y\!\downarrow$
\end{tcolorbox}}
\end{columns}
\end{frame}
...
\begin{frame}{KLEM --- Key Parameter Interpretation}
\vspace{0.2em}
\centering
\small
$Q = \mathrm{CES}\!\Big(\underbrace{\mathrm{CES}\!\big(\underbrace{A_t \cdot \mathrm{CES}(K,\,L;\,\sigma_{KL})}_{\text{KL}},\;\underbrace{\mathrm{CES}(EL,\,NEL;\,\sigma_{EL,NEL})}_{\text{E}};\;\sigma_{KLE}\big)}_{\text{KLE}},\;M;\;\sigma_{Q}\Big)$
...
\midrule
\rowcolor{SpruceGreen!8}
$\sigma_{KL}$ & $\sim$0.8 & KL & Complements ($<\!1$): capital--labor (Oberfield \& Raval) \\
...
\midrule
\rowcolor{SpruceGreen!8}
$A_t$ & calibrated & KL & Total factor productivity; matches SSP GDP \\
$s$ & 0.22 & Budget & GDP reinvestment rate \\
\rowcolor{SpruceGreen!8}
$\delta$ & 0.05 & Capital & Annual depreciation rate \\
$\ell$ & regional & Labor & Working-age share of population (ILO 2020) \\
\bottomrule
\end{tabular}
\end{frame}
...
\vspace{0.3em}
\bi
  \item Income $\uparrow$ $\;\to\;$ floorspace per capita $\uparrow$ (saturates in rich countries)
  \item Floorspace $\times$ climate (HDD/CDD) $\;\to\;$ service demand per m$^2$
  \item Service demand $\div$ carrier efficiency $\;\to\;$ final energy by carrier
\ei
...
\begin{tikzpicture}[
  ...
  \node[pt, fill=blue!12, draw=blue!50!black] (p5) at (4.7,-2.6) {H$_2$};
  ...
  \node[pt, fill=green!12, draw=green!50!black] (f2) at (8.0,-2.6) {Bio-Diesel};
  \node[pt, fill=yellow!15, draw=yellow!60!black] (f3) at (9.2,-2.6) {Electric};
  \node[pt, fill=blue!12, draw=blue!50!black] (f4) at (10.3,-2.6) {H$_2$};
...
\vspace{0.3em}
\small
\bi\setlength\itemsep{3pt}\setlength\parsep{0pt}
\item Income $\uparrow$ $\;\to\;$ service demand per capita $\uparrow$ (income elasticity $\alpha$)
\item Service demand $\div$ vehicle efficiency $\;\to\;$ final energy by carrier
\item \textbf{LCOT} = LCOE + time cost: \; vehicle cost + fuel + O\&M + $\dfrac{VOT}{speed}$
  \bi\setlength\itemsep{1pt}
  \item[\scriptsize$\bullet$] {\scriptsize Consumers value travel time --- faster/costlier modes compete with slower/cheaper ones}
  \ei
\ei
\end{frame}
...
\begin{frame}{Primary Energy Supply --- Fossil Fuels (Rogner, 1997)}
\small
...
\begin{frame}[t]{Inter-Regional Trade --- Global Market Clearing (Edmonds \& Reilly, 1985)}
\vspace{0.3em}
...
\begin{frame}[t]{Technology Choice (Modified from Clarke \& Edmonds, 1993)}
...
\begin{frame}[t]{Two-Factor Learning (Bosetti et al.\ 2006)}
\small
...
\centering
\scriptsize
Source: Rubin et al.\ 2015, \textit{Energy Policy}; Way et al.\ 2022, \textit{Joule}

\vspace{0.2em}
\begin{tabular}{l c c}
\toprule
\textbf{Technology} & \textbf{LBD Rate} & \textbf{RND} \\
\midrule
Solar PV     & 20\% & High \\
Wind         & 12\% & Medium \\
Batteries    & 19\% & High \\
...
\begin{frame}[t]{Vintage Retirement (Grubler, 1990)}
...
\vspace{-0.3em}
\bi
  \item Age-based physical degradation --- same-cohort plants retire at different ages
\ei
\vspace{-0.5em}
\centering
\includegraphics[width=\linewidth, height=0.75\textheight, keepaspectratio]{scurve.png}
\end{frame}

% ── Slide 18: Profit Shutdown ────────────────────────────────────
\begin{frame}[t]{Profit Shutdown (Calvin et al., 2019)}
\vspace{-0.3em}
\bi
  \item Economics-based early retirement --- unprofitable plants shut down regardless of age
\ei
\vspace{-0.5em}
\centering
\includegraphics[width=\linewidth, height=0.75\textheight, keepaspectratio]{profit_shutdown.png}
\end{frame}
...
% ── Validation Overview ──────────────────────────────────────────
\begin{frame}[t]{Calibration Validation}
\vspace{0.3em}
{\small
Validate GHIM against GCAM v8.2 SSP2-Ref (32 regions, 2025--2100):
\bi[leftmargin=1.2em, itemsep=3pt, topsep=4pt]
\item GDP \& Population
\item Final Energy by Sector (Industry, Buildings, Transport)
\item Final Energy by Carrier (Electricity, Gas, Liquids, Coal, Biomass, H$_2$, Heat)
\item Electricity Generation
\item Electricity Technology Shares
\ei}

\vspace{0.8em}
\begin{tcolorbox}[colback=SpruceGreen!5, colframe=SpruceGreen!70, title={\bfseries Calibration Statistics}, fonttitle=\small]
\small
\bi[leftmargin=1.2em, itemsep=2pt, topsep=2pt]
\item \textbf{CC} (Correlation Coefficient): trajectory shape agreement ($-1$ to $1$, higher better)
\item \textbf{MAPE} (Mean Absolute \% Error): average relative deviation per period
\item \textbf{MAE} (Mean Absolute Error): average absolute deviation (EJ or \%p)
\ei
\end{tcolorbox}
\end{frame}
...
\item \textbf{GDP}: CC\,=\,1.0000, MAPE\,=\,0.01\%, MAE\,=\,15.1\,B\$
\item \textbf{Population}: exogenous SSP2 projections (not calibrated)
\ei}
\vspace{0.1em}

\centering
\includegraphics[width=\linewidth, height=0.80\textheight, keepaspectratio]{val_gdp_pop.png}
\end{frame}
...
{\small
\bi[leftmargin=1.2em, itemsep=1pt, topsep=0pt]
\item \textbf{Total Final Energy}: CC\,=\,1.0000, MAPE\,=\,1.5\%, MAE\,=\,9.1\,EJ
\ei}
\vspace{0.1em}

\centering
\includegraphics[width=\linewidth, height=0.82\textheight, keepaspectratio]{val_fe_sector.png}
\end{frame}
...
{\small
\bi[leftmargin=1.2em, itemsep=1pt, topsep=0pt]
\item \textbf{Carrier Shares} (share-weighted average): CC\,=\,0.9996, MAE\,=\,0.2\,\%p
\ei}
\vspace{0.1em}

\centering
\includegraphics[width=\linewidth, height=0.82\textheight, keepaspectratio]{val_fe_carrier.png}
\end{frame}
...
{\small
\bi[leftmargin=1.2em, itemsep=1pt, topsep=0pt]
\item \textbf{Electricity Generation}: CC\,=\,1.0000, MAPE\,=\,0.8\%, MAE\,=\,1.6\,EJ
\ei}
\vspace{0.1em}

\centering
\includegraphics[width=\linewidth, height=0.82\textheight, keepaspectratio]{val_elec_gen.png}
\end{frame}
...
{\small
\bi[leftmargin=1.2em, itemsep=1pt, topsep=0pt]
\item \textbf{Technology Shares} (generation-weighted average): CC\,=\,0.9450, MAE\,=\,0.4\,\%p
\ei}
\vspace{0.1em}

\centering
\includegraphics[width=\linewidth, height=0.82\textheight, keepaspectratio]{val_elec_mix.png}
\end{frame}
...
\vspace{-0.5em}
\centering
\makebox[\textwidth][c]{\includegraphics[width=1.12\textwidth]{gantt.png}}
\end{frame}
...
\begin{frame}{Phase 2 --- Energy Services Nest (Popp, 2004)}
\begin{columns}[T]
\column{0.42\textwidth}
...
\begin{frame}[t]{Final Energy Demand for Buildings (Eom et al., 2012) --- 1/2: Floorspace}
\begin{columns}[T]
...
\begin{frame}[t]{Final Energy Demand for Buildings (Eom et al., 2012) --- 2/2: Service Intensity}
\begin{columns}[T]
\column{0.52\textwidth}
\small
\vspace{0.1em}
...
\begin{frame}[t]{Solver Iteration $F(\mathbf{x})$}
\vspace{0.1em}
\centering
\begin{tikzpicture}[
