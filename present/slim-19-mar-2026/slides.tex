...
\begin{tikzpicture}[
  ...
  % Transformation
  \node[tbox, minimum width=2.2cm] (tr_elec) at (0, -2) {Electricity\\Generation};

  % Primaries (elec-only) — grouped box
  \def\erow{1.2}  % half-width of each row
  \node[pbox, fill=green!8, draw=green!40, minimum width=0.9cm] (p_sol)  at (-\erow, -3.5) {Solar};
  \node[pbox, fill=green!8, draw=green!40, minimum width=0.9cm] (p_wind) at (0, -3.5)      {Wind};
  \node[pbox, fill=green!8, draw=green!40, minimum width=0.9cm] (p_hyd)  at (\erow, -3.5)  {Hydro};
  \node[pbox, fill=green!8, draw=green!40, minimum width=1.0cm] (p_u)    at (-0.65, -4.3)  {Uranium};
  \node[pbox, fill=green!8, draw=green!40, minimum width=1.2cm] (p_geo)  at (0.65, -4.3)   {Geothermal};
  % Group outline
  \begin{pgfonlayer}{background}
  ...
  % Carriers (right side) — compressed spacing
  \node[cbox, minimum width=1.0cm] (c_gas)  at (3.8, 0)  {Gas};
  \node[cbox, minimum width=1.0cm] (c_liq)  at (5.2, 0)  {Liquids};
  \node[cbox, minimum width=1.0cm] (c_sol)  at (6.6, 0)  {Solids};
  \node[cbox, minimum width=1.0cm] (c_heat) at (8.0, 0)  {Heat};
  \node[cbox, minimum width=1.0cm] (c_h2)   at (9.4, 0)  {H$_2$};
  \node[cbox, minimum width=1.0cm] (c_biof) at (10.8, 0) {Biofuel};
  \node[cbox, minimum width=1.0cm] (c_bio)  at (12.2, 0) {Biomass};

  % Transformation (right side) — 7 boxes, compressed
  \node[tbox, minimum width=1.3cm, font=\tiny\bfseries] (tr_gasp) at (3.5, -2)  {Gas\\Processing};
  \node[tbox, minimum width=1.3cm, font=\tiny\bfseries] (tr_ref)  at (5.0, -2)  {Oil\\Refining};
  \node[tbox, minimum width=1.3cm, font=\tiny\bfseries] (tr_sfp)  at (6.5, -2)  {Solid Fuel\\Processing};
  \node[tbox, minimum width=1.3cm, font=\tiny\bfseries] (tr_dh)   at (8.0, -2)  {District\\Heating};
  \node[tbox, minimum width=1.3cm, font=\tiny\bfseries] (tr_h2)   at (9.5, -2)  {H$_2$\\Production};
  \node[tbox, minimum width=1.3cm, font=\tiny\bfseries] (tr_bio)  at (11.0, -2) {Bio-\\refining};
  \node[tbox, minimum width=1.3cm, font=\tiny\bfseries] (tr_bmp)  at (12.5, -2) {Biomass\\Processing};
  ...
  \foreach \s in {p_coal, p_oil, p_gas, p_bio} {
    \draw[yellow!50!orange, link, opacity=0.4] (\s.north west) -- (tr_elec.south east);
  }
